...
Estrategia, diseño técnico y **Sprint 0 (esqueleto de código) completos y validados**: el producto acompaña el recorrido completo de un médico — descubrir el mejor país (con compatibilidad explicada, no un comparador), planificar un camino personalizado, ser acompañado durante la espera (Radar de Espera, el diferenciador anidado) y continuar durante toda la carrera internacional. España es el beachhead de profundidad; la arquitectura de dominio (DDD, monolito modular con 9 bounded contexts) escala a más países, rutas, especialidades e idiomas sin rediseñar el núcleo, y está protegida por pruebas arquitectónicas automatizadas (`npm run test:arch`). El backend corre de verdad: registro → perfil → motor de descubrimiento explicable, probado de punta a punta contra datos reales de España/Alemania. Ver `docs/09-recomendacion-final-6-meses.md` para la apuesta de producto, `docs/05-domain-model-ddd.md` para el diseño de dominio, y `docs/15-sprint-0-entregables.md` para el estado técnico y el plan de Sprint 1.
