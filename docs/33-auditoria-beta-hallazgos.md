...
| 5 | 2026-07-22 | Panel privado `/admin-historias.html` — login con credenciales incorrectas | El formulario de login propio muestra "Usuario o contraseña incorrectos" y deja reintentar | El primer intento con `AdminBasicAuthGuard` enviaba la cabecera `WWW-Authenticate: Basic` en el 401 — eso dispara el diálogo NATIVO de usuario/contraseña del navegador en cualquier `fetch()`, que se queda colgado indefinidamente (nunca resuelve la promesa) en vez de dejar que el JS de la página maneje el error | 🔴 Bug de Beta | **Corregido antes de commitear** — se quitó la cabecera `WWW-Authenticate` del guard; verificado con smoke test de Playwright (login incorrecto ahora responde en <1s con el mensaje esperado, login correcto funciona, filtros y exportación CSV funcionan) |
