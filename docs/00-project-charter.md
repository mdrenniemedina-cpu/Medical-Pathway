...
| Customer Discovery Sprint | **Kit de ejecución completo (protocolo, reclutamiento, métricas, decisión) + evidencia secundaria real; entrevistas 1:1 pendientes de ejecución humana** | `docs/18` a `docs/23`, ver `docs/23-informe-customer-discovery.md` |
...
**Estado actual del proyecto (desarrollo de nuevas funcionalidades suspendido a petición del founder):** el bloqueador ya no es técnico — es conseguir médicos/estudiantes reales que completen el Customer Discovery Sprint. El kit completo está listo (`docs/18-25`); la evidencia secundaria pública ya recopilada (`docs/22-discovery-evidencia-secundaria.md`) corrobora que la espera de homologación en España es un punto de dolor real y documentado (colectivo "Homologación Justa Ya", ~30.000 expedientes atascados), pero no puede sustituir las entrevistas 1:1. **Decisión tomada:** primero un piloto de 3-5 contactos cercanos para validar el propio protocolo (`docs/24-discovery-piloto-metodologia.md`), separando explícitamente entrevistas de "solo problema" (Fase A) de entrevistas que además observan el prototipo real (Fase B, `docs/25-discovery-guion-observacion-mvp.md`) — nunca mezcladas, y la evidencia piloto nunca cuenta hacia los umbrales de decisión del estudio principal. El reclutamiento completo de 10-15 (`docs/19`) queda en pausa hasta revisar el protocolo con los resultados del piloto. Sprint 2 no inicia desarrollo de nueva funcionalidad hasta entonces — ver `docs/23-informe-customer-discovery.md` para el detalle y los próximos pasos.
